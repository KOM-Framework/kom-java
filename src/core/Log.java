...
public class Log extends Logger {
	...
	public static void writeLog(String testCaseName) {
		String fileName = Log.getFinalFileName(logFolderLocation(), testCaseName);
		File logFile = new File(logFolderLocation() + "/" + fileName + ".txt");
		try {
	...
	public static void captureScreenshot(String testCaseName) {
		...
		String screenshotFileName = Log.getFinalFileName(folderLocation, testCaseName);
		try {
			FileUtils.copyFile(scrFile, new File(folderLocation + "/" + screenshotFileName + "_Screenshot.jpg"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
