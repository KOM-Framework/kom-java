 ...
 * 
 * @author Rudolf Ingerl http://mathdotrandom.blogspot.com
 */
public class Cloner {
	 ...
	 * 
	 * 
	 * This method takes an object and produces a <i>best possible</i> deep
	 * clone of the supplied param.
	 * 
	 * 
	 * 
	 * NB: Not all supplied objects can be deep cloned... this works best for
	 * beans or domain model objects.
	 * 
	 * 
	 * 
	 ...
	 * 
	 * - Must have public fields, or must have visible accessor methods for any
	 * private fields (get*; set*; is*) (default or protected fields might
	 * work...)
	 * 
	 * - Must not have any immutable classes somewhere in it's hierarchy tree
	 * (those classes should be <code>final</code> anyway...)
	 * 
	 * - Must not have any fields as anonymous classes (since they cannot have
	 * constructors)
	 * 
	 * 
	 * Supported clonable object features: - Standard objects
	 * 
	 ...
	 * 
	 * - Default: You need to place this class in the same package as the class
	 * you wish to clone
	 * 
	 * - Protected: You need to copy these two methods into lowest class in the
	 * inheritance hierarchy which contains the protected fields you wish to
	 * clone
	 * 
	 * 
	 * 
	 * This method ignores any implementation of Object.clone() or any reference
	 * to java.lang.Clonable as these are not guaranteed to be deep-cloning...
	 * 
	 * 
	 * 
	 * @param obj
	 ...
	 * @throws Exception
	 *             thrown only if bad reflective assumptions are made - not if
	 *             clone is not a 100% copy. Please make sure your clones are
	 */
	...
	private static <T> T deepClone(T clone, Object obj, Class currentClazz, T parent) throws Exception {
		...
		if (currentClazz.isArray()) {
			...
			for (int i = 0; i < length; i++) {
				Array.set(clone, i, deepClone(Array.get(obj, i))); // can't use
																	// System.arraycopy
																	// to copy
																	// across
																	// the array
		...
		if (clone == null) {
			try {
				if (currentClazz.isMemberClass()) {
					Constructor constructor = currentClazz
							.getConstructor(new Class[] { currentClazz.getDeclaringClass() });
		...
		/**
		 * Mutable Char Sequences StringBuffer, StringBuilder (Sequence will
		 * clone OK via the non-specialized code...) (CharBuffer doesn't have a
		 * no-args constructor)
		 */
		if (obj instanceof CharSequence) {
			 ...
			 */
			if (currentClazz.equals(java.lang.StringBuffer.class)
					|| currentClazz.equals(java.lang.StringBuilder.class)) {
				Method append;
				try {
		...
		for (Field field : fields) {
			...
			// Enable this check - depending on how you want to handle
			// transient...
			...
			try { // attempt to copy across accessible fields
				...
				// consume - can't set value on a field with the default or
				// protected access modifier if that field is not in scope of
				// THIS METHOD's CLASS (not the class invoking this method!1)
			} catch (Throwable t) {
				t.printStackTrace();
				throw new Exception("Unable to clone field: " + field.getName() + ", of type: " + field.getType()
						+ " on class: " + currentClazz);
			}
		}
		...
		for (Method method : methods) {
			...
			if (!method.getReturnType().equals(Void.TYPE) && method.getParameterTypes().length == 0) {
				...
				for (Method setterMethod : methods) {
					...
					if (setterMethod.getName().toLowerCase().equals("set" + fieldName)
							&& setterMethod.getReturnType().equals(Void.TYPE)
							&& setterMethod.getParameterTypes().length == 1
							&& setterMethod.getParameterTypes()[0].equals(method.getReturnType())) {
						try {
							setterMethod.invoke(clone,
									deepClone(null, method.invoke(obj, new Object[] {}), null, clone));
						} catch (Throwable t) {
							// consume - either we don't have access to that
							// method, or we're sending an incorrect param
